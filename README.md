...
### ⚠️ 异常数据处理
当CSV中包含"异常"列时：
- **图表显示**: 异常数据点用红色×标记（DIF）或红色◆标记（RAW）
- **连线断开**: 异常数据点不与前后数据连接，线段自动断开
- **Hover提示**: 鼠标悬停时显示"⚠️ 异常数据"提示
- **检测条件**: 异常列中任意非空值（除0、空字符串外）都视为异常数据

### 示例CSV
```csv
用户名,MAC地址,时间,DIF百分比,RAW百分比,是否入耳,左右耳,异常
user1,AA:BB:CC:DD:EE:FF,2024-01-15 14:30:01,75.5,82.3,是,左耳,
user1,AA:BB:CC:DD:EE:FF,2024-01-15 14:30:02,76.2,83.1,是,左耳,
user2,BB:CC:DD:EE:FF:AA,2024-01-15 14:30:03,45.3,52.7,否,右耳,error
user2,BB:CC:DD:EE:FF:AA,2024-01-15 14:30:04,100.0,105.2,是,右耳,
```
...
- ✨ **新增数据分页**
  ...
  - 图表与当前页数据联动显示
- ✨ **新增异常数据支持**
  - CSV中包含"异常"列时，异常数据点显示为红色标记
  - 异常数据点连线自动断开（DIF用×，RAW用◆）
  - Hover提示中显示异常数据标记
- 🔧 **代码优化**
  - 智能时间列检测和处理
  - 提升大数据量处理性能
